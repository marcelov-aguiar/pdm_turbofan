...
equipment_name = ['FD001', 'FD003']
experiment_name = 'FD0001_FD003'
...
# faz a contagem para recalcular unit_number
i = 0
for equipment in equipment_name:
    logger.info(f"Lendo os dados de treino do equipamento {equipment}.")
    path_dataset_train = \
        str(path_preprocessing_output.joinpath(f"train_{equipment}.csv"))

    df_aux = pd.read_csv(path_dataset_train)
    df_aux['unit_number'] = df_aux['unit_number'] + i
...
equipment = 'FD001'
logger.info(f"Lendo os dados de teste do equipamento {equipment}.")

path_dataset_test = \
    str(path_preprocessing_output.joinpath(f"test_{equipment}.csv"))
...
with mlflow.start_run(run_name='XGBoost'):
    ...
    mlflow.xgboost.log_model(model.regressor_['regressor'],
                             'TURBOFAN',
                             signature=signature)
