...
class FormatData(ManipulateData):
    ...
    def format_raw_data(self, input_file_name: str) -> None:
        ...
        path_output_format_data = os.path.join(get_path_exploratory_data, output_file_name)
    ...
    def get_format_data(self, output_file_name: str) -> pd.DataFrame:
        ...
        try:
            df_data = pd.read_csv(os.path.join(get_path_exploratory_data,
                                               f"{output_file_name}_format.csv"))
        except Exception as e:
            print(f"Dado não encontrado. Execute método format_raw_data para {output_file_name}.")
            print(e)
